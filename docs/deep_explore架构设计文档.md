...
**注意：** 当场景中仅包含一个动作时（如上例），其功能等同于之前的 action 模式。这种设计简化了框架结构，同时保持了原有的功能。
